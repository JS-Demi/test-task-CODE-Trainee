...
import { Departments, Filters } from '../types/types'
import { nanoid } from 'nanoid'

interface SearchLayoutProps {}

const SearchLayout: FC<SearchLayoutProps> = () => {
	const filters: Record<Departments, string> = {
		[Departments.All]: 'Все',
		[Departments.Analytics]: 'Аналитика',
		[Departments.Back_office]: 'Бэк-офис',
		[Departments.Design]: 'Дизайн',
		[Departments.Management]: 'Менеджмент',
		[Departments.Support]: 'Техподдержка',
		[Departments.Backend]: 'Backend',
		[Departments.Frontend]: 'Frontend',
		[Departments.Android]: 'Android',
		[Departments.iOS]: 'iOS',
		[Departments.HR]: 'HR',
		[Departments.PR]: 'PR',
		[Departments.QA]: 'QA',
	...
	console.log(filters)
	const selected: Departments = Departments.All
	return (
		<>
			<Heading as='h2' size='lg' pt={'12px'} px={'16px'}>
				Поиск
			</Heading>
			<Box px={'16px'}>
				<InputGroup>
					<InputLeftElement>
						<Search2Icon color={'#C3C3C6'} />
					</InputLeftElement>
					<Input
						variant={'outline'}
						borderRadius={'16px'}
						...
						_placeholder={{ color: '#C3C3C6' }}
					/>
					<InputRightElement>
						<Icon viewBox='0 0 21 12'>
							<path
			...
			<Flex flexDirection={'column'}>
				<Stack px={'16px'} color={'#97979B'} direction={'row'} align={'center'} gap={0}>
					{Object.entries(filters).map(([endpoint, name]) => (
						<FilterButton key={nanoid()} selected={selected} endpoint={endpoint} name={name} />
					))}
				</Stack>
				<Divider color={'#C3C3C6'} opacity={1} />
			</Flex>
		</>
	)
}
